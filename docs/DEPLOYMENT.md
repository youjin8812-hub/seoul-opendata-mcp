...
- **토큰 없이 공개한다면 호출 제한을 반드시 켜 둘 것.** 서울 열린데이터광장 카탈로그 API(`SearchCatalogService`)는 1회 1,000건 상한만 있고 **호출 횟수 제한이 없으므로 인증키 한도가 소진될 걱정은 없다.** (1일 1,000회 제한은 실시간 지하철 API에만 적용되며 이 서버는 호출하지 않는다.) 다만 `MCP_AUTH_TOKEN`을 비우면 누구나 호출할 수 있어 서버 자원이 그대로 노출되고, 서울시 서버에도 불필요한 부하가 간다. 기본값(분당 20 / 시간당 200)이 켜져 있으므로 `MCP_RATE_LIMIT_*`를 `0`으로 끄지 않는 한 보호된다. 추천 1회가 카탈로그 API를 최대 8회 호출한다는 점을 감안해 한도를 정한다.
