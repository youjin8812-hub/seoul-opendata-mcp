...
function defaultRetryOn(err, response) {
    if (isPermanentError(err))
        return false;
    if (response) {
        if (response.status >= 400 && response.status < 500)
            return false; // 4xx 영구 오류
...
export async function fetchWithRetry(url, init, options = {}) {
    const { maxAttempts = 3, baseDelayMs = 500, timeoutMs = 8000, retryOn = defaultRetryOn, fetchFn = fetch, } = options;
    let lastError;
    let lastResponse;
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        try {
            ...
            if (attempt < maxAttempts) {
                const delayMs = baseDelayMs * Math.pow(2, attempt - 1);
                logger.warn(`네트워크 오류 재시도 ${attempt}/${maxAttempts} — ${delayMs}ms 대기`, { error: lastError instanceof Error ? lastError.message : String(lastError) });
                await sleep(delayMs);
            }
        }
    }
