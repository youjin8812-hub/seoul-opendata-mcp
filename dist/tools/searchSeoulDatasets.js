...
const searchCache = new MemoryCache(3 * 60 * 1000);
export async function searchSeoulDatasetsForTool(input) {
    const { query, page = 1, limit = 10, orgName, division } = input;
    const cacheKey = normalizeCacheKey(`search|${query}|${page}|${limit}|${orgName ?? ""}|${division ?? ""}`);
    ...
    const { items: rawItems, totalCount } = await searchSeoulCatalog({ keyword: query, orgName, start, end }, serviceKey);
...
//# sourceMappingURL=searchSeoulDatasets.js.map