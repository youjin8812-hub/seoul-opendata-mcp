...
const recentUpdatesCache = new MemoryCache(3 * 60 * 1000);
/** SearchCatalogService 1회 호출 최대 허용치 — 정렬 대상 표본을 최대한 넓게 확보 */
const FETCH_SIZE = 1000;
export async function listSeoulRecentUpdates(input) {
    const { keyword, orgName, division, apiOnly = false, limit = 10 } = input;
    const cacheKey = normalizeCacheKey(`recent|${keyword ?? ""}|${orgName ?? ""}|${division ?? ""}|${apiOnly}|${limit}`);
    const cached = recentUpdatesCache.get(cacheKey);
    if (cached)
        return cached;
    const serviceKey = getServiceKey();
    const { items: rawItems, totalCount } = await searchSeoulCatalog({ keyword, orgName, start: 1, end: FETCH_SIZE }, serviceKey);
...
//# sourceMappingURL=listSeoulRecentUpdates.js.map