 ...
 */
export function extractKeywords(ideaText, domainHint) {
    ...
    // 도메인 힌트 추가
    if (domainHint) {
        const hintTokens = tokenize(domainHint).filter((t) => !STOP_WORDS.has(t));
        hintTokens.forEach((t) => {
            expanded.add(t);
            const expansions = DOMAIN_EXPANSIONS[t];
            if (expansions)
                expansions.slice(0, 2).forEach((e) => expanded.add(e));
        });
    }
    // 중복 제거 후 최대 8개
