 ...
 *
 * 공개 서버는 인증키 하나의 일일 호출 예산을 모두가 나눠 쓴다. 카탈로그
 * 메타데이터는 거의 변하지 않으므로 TTL을 길게 잡는 것이 예산을 아끼는
 * 가장 효과적인 수단이다.
 *
 * - 일반 쿼리: 1시간 TTL
 * - 실시간 키워드 포함 쿼리: 5분 TTL (최신 데이터 중요)
 * - 데이터셋 상세 정보: 24시간 TTL (변경 빈도 매우 낮음)
 */
...
export const TTL = {
  REALTIME: 5 * 60 * 1000,           // 5분 — 실시간 키워드 쿼리
  DEFAULT: 60 * 60 * 1000,           // 1시간 — 일반 추천/검색
  DETAIL: 24 * 60 * 60 * 1000,       // 24시간 — 데이터셋 상세 정보
} as const;
