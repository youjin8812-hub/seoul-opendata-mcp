 ...
 *
 * 인증 없이 공개 운영하는 서버의 자원 보호와 남용 방지가 목적이다.
 *
 * 서울시 인증키 한도 때문은 아니다. 이 서버가 호출하는 카탈로그 API
 * (SearchCatalogService)는 1회 1,000건 상한만 있고 호출 횟수 제한이 없다.
 * 1일 1,000회 제한이 걸리는 실시간 지하철 API는 호출하지 않는다.
 * 다만 추천 1회가 카탈로그를 최대 8회 호출하므로, 제한 없이 열어두면
 * 서버 자원과 서울시 서버 양쪽에 불필요한 부하가 갈 수 있다.
 *
