...
function mapRow(row: Record<string, unknown>): RawSeoulCatalogItem {
  return {
...
function parseXmlResultFallback(
  text: string
...
export async function searchSeoulCatalog(
  ...
  try {
    ...
    if (!fallback) {
      throw new Error(
        `서울 열린데이터광장 API 응답을 해석할 수 없습니다 (HTTP ${res.status})`
      );
    }
    parsed = fallback;
  }
  ...
  const wrapper =
    (parsed[SERVICE_NAME] as Record<string, unknown> | undefined) ?? parsed;
  ...
  if (code !== "INFO-000") {
    ...
    if (code === "INFO-100" || message.includes("인증키")) {
      throw new Error(
        `서울 열린데이터광장 인증키 오류: ${message}. data.seoul.go.kr에서 발급받은 인증키가 맞는지 확인하세요.`
      );
    }
  ...
  const rows = Array.isArray(wrapper["row"])
    ? (wrapper["row"] as Record<string, unknown>[])
    : [];
