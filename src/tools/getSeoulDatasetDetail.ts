...
function extractInfId(input: string): string | null {
  const trimmed = input.trim();
  if (/^OA-[\w-]+$/i.test(trimmed)) return trimmed;
...
export async function getSeoulDatasetDetail(
  detailUrl: string
  ...
  const result: DatasetDetailOutput = {
    ...
    endpoints: [],
    authMethod:
      "서울 열린데이터광장 인증키 — data.seoul.go.kr 마이페이지에서 발급 후 URL 경로에 포함",
