...
import type { BrmClassification, BrmPrimaryCategory, RawSeoulCatalogItem } from "../types/index.js";

const BRM_CATEGORIES: BrmPrimaryCategory[] = [
  "보건",
  "일반행정",
  "문화/관광",
  "산업/경제",
...
const NORMALIZATION_MAP: Record<string, BrmPrimaryCategory> = {
  문화관광: "문화/관광",
  산업경제: "산업/경제",
  인구가구: "인구/가구",
  주택건설: "주택/건설",
...
function normalize(raw: string): BrmPrimaryCategory | null {
  const trimmed = raw.trim();
  if (!trimmed) return null;
  if ((BRM_CATEGORIES as string[]).includes(trimmed)) {
    return trimmed as BrmPrimaryCategory;
  }
  return NORMALIZATION_MAP[trimmed] ?? null;
}

export function classifyBrm(raw: RawSeoulCatalogItem): BrmClassification {
