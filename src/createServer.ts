/**
 * Seoul Open Data API Finder — MCP 서버 팩토리
 *
 * 전송 계층(stdio / Streamable HTTP)과 무관한 서버 본체를 정의한다.
 * - stdio 진입점:  src/server.ts       (로컬 MCP 클라이언트용)
 * - HTTP 진입점:  src/httpServer.ts   (Fly.io 등 원격 배포용)
 *
...
export const TOOL_DEFINITIONS = [
  {
    ...
    description:
      ...
      "응답은 데이터명·점수·담당부서·갱신주기가 포함된 마크다운 표로 옵니다 — 사용자에게 답할 때 이 표를 요약해서 열을 빼지 말고, 표와 점수 근거를 그대로 옮겨 보여주세요.",
    inputSchema: {
      type: "object",
      properties: {
        ideaText: {
  ...
  },
  {
    ...
    description:
      "서울시 데이터셋 상세 메타데이터를 조회합니다. 서울 열린데이터광장 카탈로그 API로 제공기관, 담당부서, 갱신주기, 최종갱신일, 제공형식(SRV_TYPE)을 반환합니다. " +
    ...
    inputSchema: {
      ...
      properties: {
        detailUrl: {
          type: "string",
          description:
            "data.seoul.go.kr 데이터셋 상세 페이지 URL 또는 서비스 ID (예: https://data.seoul.go.kr/dataList/OA-15529/S/1/datasetView.do 또는 OA-15529)",
        },
      },
      required: ["detailUrl"],
    },
  ...
  },
  {
    ...
    description:
      "최근 갱신된 서울시 데이터셋을 최종갱신일(DATA_LT_NM) 기준 내림차순으로 조회합니다. " +
...
/**
 * 도구 핸들러가 등록된 MCP 서버 인스턴스를 새로 만든다.
 * HTTP 전송에서는 요청(세션)마다 독립 인스턴스를 만들어 상태 간섭을 막는다.
 */
export function createSeoulMcpServer(): Server {
