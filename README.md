...
- **응답은 마크다운 표로 정리되어 나온다** — 데이터명(상세페이지 링크) · 제공형식 · 갱신주기 · 최종갱신 · 제공기관 · 담당부서 · 점수. 원시 JSON만 돌려주면 클라이언트마다 요약이 달라져 항목이 누락되므로, 표를 고정한 마크다운은 `content`로, 구조화된 원본 데이터는 `structuredContent`로 분리해 보낸다 (`refine_seoul_recommendations` 입력용). JSON을 `content`에 함께 넣지 않는 이유는, 호출하는 어시스턴트가 표 대신 그 JSON을 보고 자기 방식대로 답을 재구성해 표가 무시되는 문제를 막기 위해서다
