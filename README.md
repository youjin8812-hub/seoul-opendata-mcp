...
**📊 규모와 구성**
- **카탈로그 규모**: 공공데이터 8,251건 (플랫폼 공식 통계)
- **제공 주체**: 서울시 본청 · 자치구 및 자치구 산하 · 산하기관 · 사업소가 한 카탈로그에 통합 등록
- **제공 형식**: 한 데이터셋이 여러 형식으로 동시 제공되므로 형식별 건수의 합은 총건수를 넘는다
  - OpenAPI 5,631 · SHEET 7,331 · FILE 1,184 · CHART 1,883 · MAP 124 · LINK 320 · LOD 91
...
**📈 카탈로그 실측 분포** (본 프로젝트가 `SearchCatalogService` 전수 조회로 직접 확인한 값)
...
> 플랫폼 전체 API 호출량·이용기관 구성 등 서울시 운영 통계는 이 카탈로그 API로 조회할 수 없어 본 문서에서는 다루지 않는다.
...
**❌ 기존 방식의 한계**
1. **정확한 서비스명을 알아야 검색된다**: 카탈로그 검색은 키워드 매칭이라, 만들려는 서비스만 머릿속에 있고 데이터명을 모르면 출발점이 없음
2. **파일 vs API 혼재**: "생활인구"로 검색해도 File·Sheet·API·Chart·Map이 한 목록에 섞여 나옴
3. **최신성 판단 어려움**: 목록만 봐서는 지금도 갱신되는 데이터인지, 몇 년째 멈춘 데이터인지 구분되지 않음
4. **형식·담당부서 확인에 추가 이동**: API 여부와 담당부서·연락처를 알려면 데이터셋마다 상세페이지로 들어가 확인해야 함
5. **외부 포털 색인의 누락**: data.go.kr 색인만 믿으면 실제 존재하는 API를 "없다"고 판단하게 됨 (3장 도입 배경의 실제 사례)
...
**💰 사용 흐름 비교**
- 기존: 데이터광장 접속 → 키워드 검색 → 목록에서 형식 구분 → 데이터셋마다 상세페이지 진입 → 담당부서 확인
- 이제: AI 대화창에 한 문장 입력 → 점수·근거가 붙은 후보 목록 + 형식·갱신주기·담당부서를 한 번에 회신 (추천 질의 실측 91ms)
