...
- 오픈API는 한 번에 최대 1,000건까지 요청 가능 (초과 시 나누어 호출)
- 실시간 지하철 오픈API는 하루 최대 1,000건
- 이용 제약 없이 계속 쓰려면 활용사례(갤러리)에 인증키와 함께 콘텐츠 등록 필요
...
키워드 추출은 조사·어미와 요청 표현("만들게", "추천해줘")을 제거한 뒤, 사전 표제어를 **부분 문자열로 포함하는 합성 신조어까지 확장**한다. 예를 들어 "그늘맵"은 표제어 "그늘"에 연결되어 `그늘 · 그늘막 · 무더위쉼터 · 폭염 · 가로수 · 녹지`로 확장된다. 원문 키워드는 `coreKeywords`, 파생 유사어는 `expandedKeywords`로 구분되어 점수화에서 다른 가중을 받는다 (총 최대 14개, 검색은 상위 8개 키워드로 병렬 수행).
...
- **도메인 적합도 계산**: 매칭 위치별 가산(제목 > 태그 > 본문) 후 40점 상한. 원문 키워드(제목 12 · 태그 5 · 본문 3)가 확장 유사어(7 · 3 · 2)보다 높은 가중을 받는다. 매칭 비율 방식이 아니라 가산 방식이므로 **유사어를 늘려도 점수가 희석되지 않는다**
