...
키워드 일치가 관련도 65점 중 47점이라 **유사어 확장 품질이 추천 결과를 좌우한다.** 관련도 게이트가 있어 유사어가 부실하면 후보가 아예 0건이 되므로, 세 경로로 유사어를 모은다.
...
- **점수 배점 (95점 만점)**: **질문 관련도 65점 + 데이터 활용도 30점**. 점수·관련도·활용도는 한 곳에서 계산되며 `score === relevanceScore + qualityScore`가 항상 성립한다
- **관련도가 순위를 지배한다**: 관련도(65) > 활용도(30)이므로, 주제가 안 맞는 데이터가 OpenAPI·실시간·최신이라는 이유만으로 유관 데이터를 밀어내는 일이 구조적으로 불가능하다
- **관련도 게이트**: ① 질의의 주제어를 제목·태그에서 하나도 못 맞춘 데이터 ② 키워드 커버리지가 그 질의의 최고 매칭 대비 40%에 못 미치는 데이터 ③ 총점 35점 미만 — 셋 중 하나라도 걸리면 결과에서 제외된다. 전부 탈락하면 빈손 대신 제목·태그에 걸린 후보를 완화 기준으로 되살린다
...
### 점수 산정 (95점 = 관련도 65 + 활용도 30)

| 질문 관련도 (65) | 배점 | 적용 조건 |
|---|---|---|
| 키워드 일치 | 47 | 키워드가 있는 질의 |
| 정책분야(BRM) 일치 | 6 | 질의에서 정책분야가 추론될 때 |
| 지역(자치구) 일치 | 5 | 질의가 자치구를 지목했을 때 |
| 실시간성 요구 일치 | 4 | 실시간을 요청했을 때 |
| 제공기관 조건 일치 | 3 | `orgName`을 지정했을 때 |
...
**조건부 항목은 요구가 있을 때만 배점에 들어갑니다.** 자치구를 지목하지 않은 질의에서 지역 5점은 전 건 0점이라 순위에 무의미하고 만점 대비 비율만 왜곡하므로, 적용 가능한 항목의 배점 합을 분모로 정규화합니다. 덕분에 질의 조건을 온전히 충족한 데이터는 어떤 질의에서든 관련도 65점(총점 95점)에 도달할 수 있습니다.

#### 키워드 일치를 계산하는 방식

| 장치 | 하는 일 |
|---|---|
| **IDF 가중** | 후보군 안에서 흔한 단어일수록 가중을 낮춘다. "현황"·"정보"·"위치"처럼 카탈로그 어디에나 있는 말은 가중이 바닥으로 내려가, 사람이 범용어 목록을 손으로 관리하지 않아도 질의마다 자동으로 조정된다 |
| **커버리지** | "맞은 키워드 가중 합 ÷ 변별력 있는 전체 키워드 가중 합". 질의를 얼마나 충족했는지가 그대로 점수가 되므로, 키워드 2개 중 1개만 맞은 데이터와 2개 다 맞은 데이터가 구분된다 |
| **매칭 위치** | 제목 100% · 태그(정책분야) 60% · 본문(제공기관·부서명) 25%. 2글자 이하 키워드는 부서명 우연 일치를 아예 인정하지 않는다 |
