...
키워드 일치가 관련도 70점 중 44점이라 **유사어 확장 품질이 추천 결과를 좌우한다.** 관련도 게이트가 있어 유사어가 부실하면 후보가 아예 0건이 되므로, 세 경로로 유사어를 모은다.
...
- **점수 배점 (100점 만점)**: **질문 관련도 70점 + 데이터 활용도 30점**. 점수·관련도·활용도는 한 곳에서 계산되며 `score === relevanceScore + qualityScore`가 항상 성립한다
- **관련도가 순위를 지배한다**: 관련도(70) > 활용도(30)이므로, 주제가 안 맞는 데이터가 OpenAPI·실시간·최신이라는 이유만으로 유관 데이터를 밀어내는 일이 구조적으로 불가능하다
- **같은 뜻이면 같은 답이 나온다**: 어순·조사·말투가 달라도 추천과 점수가 동일하다 (아래 "질의 표현 불변성")
- **관련도 게이트**: ① 질의의 주제어를 제목에서 하나도 못 맞춘 데이터 ② 키워드 커버리지가 그 질의의 최고 매칭 대비 40%에 못 미치는 데이터 ③ 총점 37점 미만 ④ 자치구를 지목한 질의에서 다른 자치구 데이터 — 하나라도 걸리면 결과에서 제외된다. 전부 탈락하면 빈손 대신 제목에 걸린 후보를 완화 기준으로 되살린다
- 검색 결과는 있으나 게이트를 통과한 데이터가 없으면 억지 추천 대신 `warning`으로 안내
- 각 추천 결과에 `scoreBreakdown`(관련도·활용도·충족률·일치 키워드·근거 문장), `brm`(정책분야 분류), `organization`(제공기관 유형 분류)이 함께 반환됨
...
### 점수 산정 (100점 = 관련도 70 + 활용도 30)

배점은 이 카탈로그의 실측 특성에서 도출했습니다. 일반적인 검색 가중치가 아니라 **서울 열린데이터광장이 실제로 어떻게 생겼는지**에 맞춘 값입니다.

| 카탈로그 실측 특성 | 배점에 반영한 방식 |
|---|---|
| **설명 필드가 없다** — 응답 텍스트는 서비스명·제공기관·제공부서·시스템명뿐이고, 검색 API 자체가 서비스명 부분일치로만 동작한다 | 제목이 사실상 유일한 의미 신호 → **키워드 44점**(관련도의 63%). 본문(기관·부서명)은 우연 일치가 대부분이라 15%만 인정 |
| **소분류(MAP_CATE_NM)가 12개 정책분야와 전 건 1:1** — 누락 없는 유일한 큐레이션 축 | **정책분야 10점**. 제목 글자만 겹치고 주제는 다른 데이터를 걸러낸다 (주차장 질의에서 '교통'과 '주차위반 과태료(일반행정)'를 구분) |
| **자치구 데이터가 2,134건(26%)** — 25개 구가 같은 주제를 각자 등재 | **지역 적합도 8점을 항상 적용** + 근사중복 체감 |
| **갱신주기 실제 값이 8종** (일간·수시·주간·월간·분기별·반기별·연간·주기없음) | 실측 값에 맞춘 주기 점수. "주기없음"을 주간으로 오인하던 버그 수정 |

| 질문 관련도 (70) | 배점 | 적용 조건 |
|---|---|---|
| 키워드 일치 (서비스명 중심) | 44 | 키워드가 있는 질의 |
| 정책분야(BRM) 일치 | 10 | 질의에서 정책분야가 추론될 때 |
| 지역 적합도 | 8 | **항상** |
| 실시간성 요구 일치 | 4 | 실시간을 요청했을 때 |
| 제공기관 조건 일치 | 4 | `orgName`을 지정했을 때 |

| 데이터 활용도 (30) | 배점 |
|---|---|
| 제공형식 (OpenAPI 10 / 파일 5 / 미상 1) | 10 |
| 갱신주기 | 8 |
| 최신성 (최종갱신일) | 8 |
| 메타정보 충실도 (기관·부서·문의처·상세페이지) | 4 |

**하지 않기로 한 것 — 이용량 가중.** 이 카탈로그는 교통 한 분야가 전체 이용의 68.9%를 차지할 만큼 수요가 쏠려 있습니다. 인기 분야를 가산하면 "있는 줄 몰라서 못 쓰는 데이터"를 찾아준다는 이 도구의 목적과 정반대로 작동합니다. 쏠림은 보정 대상이지 증폭 대상이 아닙니다.

**축을 겹치지 않게 나눕니다.** 주제는 제목(키워드), 분야는 소분류(정책분야), 장소는 자치구(지역), 출처는 제공기관이 각각 전담합니다. 예전에는 소분류가 키워드 텍스트에도 들어가 있어 '교통' 질의에 교통 분야 데이터 전체가 키워드 점수와 분야 점수를 이중으로 받았습니다.

**조건부 항목은 요구가 있을 때만 배점에 들어갑니다.** 실시간을 요청하지 않은 질의에서 실시간 4점은 전 건 0점이라 순위에 무의미하고 만점 대비 비율만 왜곡하므로, 적용 가능한 항목의 배점 합을 분모로 정규화합니다. 덕분에 질의 조건을 온전히 충족한 데이터는 어떤 질의에서든 관련도 70점(총점 100점)에 도달할 수 있습니다.

#### 지역 적합도 — 자치구 26%에 대한 대응

| 질의 | 서울시 전체 데이터 | 지목한 자치구 | 다른 자치구 |
|---|---|---|---|
| "강남구 주차장" | 0.5 (강남구를 포함하므로) | 1.0 | **제외** |
| "주차장" | 1.0 | 0.5 (한 구만 담으므로) | 0.5 |

자치구명은 키워드에서 걷어내 지역 배점으로 넘깁니다. 주제(주차장)와 장소(강남구)를 갈라 놓으면 "강남구 주차장"·"주차장 강남구"·"강남 지역 주차장"이 모두 같은 질의로 수렴합니다.

자치구를 지목하지 **않은** 질의에서는 자치구명을 걷어낸 제목이 같은 그룹의 2번째부터 관련도를 25%씩 체감시킵니다. 25개 구가 같은 데이터를 각자 올리는 탓에 상위 5건이 "강남구 공영주차장 / 노원구 공영주차장 / 마포구 공영주차장…"으로 채워지는 것을 막기 위해서입니다. 감추지는 않습니다 — 특정 자치구를 찾는 사용자에게는 그 구의 데이터가 답이기 때문입니다.

#### 정책분야 추론 — 사전 + 카탈로그 자신의 분류

사전만 쓰면 사람이 계속 채워 넣어야 하고 신조어를 모릅니다("그늘맵"). 그래서 두 근거를 합칩니다.

1. **사전** — 분야별 단서 어휘. 단서가 2개 이상이거나 사용자가 분야명을 직접 말했을 때만 채택 (틀린 추론으로 정답을 깎지 않기 위해)
2. **후보군 분포** — 질의어를 제목에 담은 후보들이 실제로 어느 분야에 등재돼 있는지. 소분류는 전 건이 채워진 공식 값이라, 이 분포가 곧 "이 질의가 어느 분야를 찾는 질문인가"에 대한 카탈로그 자신의 답입니다. 표본 5건 이상 · 점유율 30% 이상일 때만 채택

분야가 **미분류**인 데이터는 이 항목을 아예 적용하지 않습니다. 없는 정보를 이유로 감점하지 않기 위해서입니다.

#### 키워드 일치를 계산하는 방식

| 장치 | 하는 일 |
|---|---|
| **IDF 가중** | 후보군 안에서 흔한 단어일수록 가중을 낮춘다. 가장 변별력 있는 키워드를 1.0으로 두고 나머지를 상대적으로 재므로, 후보군 크기가 달라져도 기준이 흔들리지 않는다 |
| **커버리지** | "맞은 키워드 가중 합 ÷ 변별력 있는 전체 키워드 가중 합". 질의를 얼마나 충족했는지가 그대로 점수가 되므로, 키워드 2개 중 1개만 맞은 데이터와 2개 다 맞은 데이터가 구분된다 |
| **매칭 위치** | 제목 100% · 본문(제공기관·부서명) 15%. 2글자 이하 키워드는 부서명 우연 일치를 아예 인정하지 않는다 |
| **원문 > 유사어** | 확장 유사어는 가장 약한 원문 키워드보다도 가볍다. 사용자가 쓴 단어가 흔하다는 이유로 바닥 가중을 받는 사이 유사어가 순위를 좌우하는 역전을 막는다 |
| **유사어는 AND가 아니라 OR** | 원문 키워드는 전부 충족해야 하지만("따릉이 대여소"), 유사어는 같은 개념의 다른 이름이므로 가장 잘 맞은 하나를 온전히 인정하고 그다음부터 절반씩 체감시킨다. "그늘막·무더위쉼터·폭염저감시설"을 전부 담은 데이터를 요구하는 건 말이 안 되기 때문이다 |
| **주제어 필수** | 최고 가중의 절반 이상인 "주제어"를 제목에서 하나도 못 맞추면 탈락. "도서관 현황 정보" 질의에서 '현황·정보'만 맞은 상수도 데이터를 걸러낸다 |
| **미검출 키워드 제외** | 후보군에 한 번도 없는 키워드(예: 카탈로그에 없는 신조어 "그늘맵")는 분모에서 뺀다. 남겨 두면 나머지를 다 맞춘 정답 데이터까지 커버리지가 깎여 게이트에 걸린다 |

### 질의 표현 불변성 — 같은 뜻이면 같은 답

추출한 키워드로 카탈로그를 검색해 후보군을 만들고, 그 후보군을 기준으로 IDF 가중을 계산합니다. 그래서 **키워드가 하나만 달라져도 후보군 → 가중 → 점수가 연쇄적으로 흔들립니다.** 어순만 바꿔 물었는데 추천이 달라지면 컨설팅 근거로 제시한 점수를 재현할 수 없어 업무에 쓸 수 없습니다.

```
"따릉이 대여소 현황"            ┐
"대여소별 따릉이 현황 알려줘"      ├→ 원문 키워드 [대여소, 따릉이] → 동일한 추천·동일한 점수
"따릉이 대여소가 어떤지 데이터 찾아줘" │
"따릉이 대여소 위치 정보가 궁금해"  ┘
```

표현 차이를 흡수하는 장치는 다섯 가지입니다.

| 장치 | 흡수하는 차이 |
|---|---|
| **의문사·요청 동사 접두 판정** | "어떤지"·"어디에"·"몇 개나"·"궁금해"·"추천해줘" — 활용형까지 한 번에 |
| **조사·접미사 정규화** | "대여소별"→"대여소", "현황이"→"현황". 주격조사 '이/가'는 받침 규칙과 사전으로 가려내 **"따릉이"를 "따릉"으로 자르지 않는다** |
| **범용어 제거** | "현황"·"정보"·"위치"·"자료"는 표현마다 붙었다 떨어졌다 하는데 변별력은 없다. 키워드에서 아예 뺀다 (실시간 요구는 따로 인식하므로 의미가 사라지지 않는다) |
| **정규 키워드 순서** | 문장에 나온 순서가 아니라 "구체적인 말 먼저 → 가나다순". 검색 쿼리는 앞쪽 8개만 쓰이므로, 어순이 후보군을 바꾸지 못하게 한다 |
| **확정적 정렬** | 동점은 관련도 → 제공형식 → 최신성 → 서비스 ID 순으로 가른다. 후보군이 어떤 순서로 도착해도 결과가 같다 |

캐시 키도 원문이 아니라 **추출된 정규 키워드 집합**으로 잡아, 같은 질의는 같은 결과를 재사용합니다.

`relevanceReasons`/`qualityReasons`에 각 배점이 부여된 근거를, `matchedKeywords`에 실제로 일치한 키워드를 함께 반환합니다.
