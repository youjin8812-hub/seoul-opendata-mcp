...
- **응답은 마크다운 표로 정리되어 나온다** — 데이터명(상세페이지 링크) · 제공형식 · 갱신주기 · 최종갱신 · 제공기관 · 담당부서 · 점수. 원시 JSON만 돌려주면 클라이언트마다 요약이 달라져 항목이 누락되므로, 표를 고정하고 구조화된 JSON을 두 번째 블록으로 함께 보낸다 (`refine_seoul_recommendations` 입력용)
